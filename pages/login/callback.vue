...
<template>
  <div class="min-h-screen flex items-center justify-center bg-modernity p-4">
    <div class="bg-care rounded-lg shadow-xl p-8 w-full max-w-md text-center">
      ...
      <div v-else-if="error" class="text-energy">
        <p class="mb-4">{{ error }}</p>
        <button 
          @click="() => navigateTo('/')" 
          class="px-4 py-2 bg-security text-care rounded hover:bg-currency transition-colors"
        >
          Return to Home
        </button>
      </div>
